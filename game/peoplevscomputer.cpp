...
void PeopleVsComputer::mouseReleaseEvent(QMouseEvent *ev)//鼠标点击棋盘并释放鼠标时触发
{
...
{
    // 判断鼠标点击处是否为在放置棋子的有效范围
    // 将pt转化成棋子的行列值
    // 判断这个行列值上面有没有棋子
    int row, col;
    if(QpointToRowCol(pt,row,col)&&chess[row][col]==0)
    {
        placeTheChess(row,col);//放置棋子
        if(isWin(row,col))
        {
...
int PeopleVsComputer::positionValue(int row, int col)//计算该点的最大价值
{
...
int PeopleVsComputer::chessType(int n, int p, int q)//返回空点p q在n方向上的棋型号，n为1-8方向，从右顺时针开始数
{
    int k, m = 0;
    /* 棋型号注解:  己活000-003 己冲010-013 对活100-103 对冲110-113
        己空活020-023 己空冲030-033 对空活120-123 对空冲130-133 空-1 边界冲-2 边界空冲-3*/
...
void PeopleVsComputer::goStep(int n, int *i, int *j)//在n方向上对坐标i j移位，n为1-8方向，从右顺时针开始数
{
    switch (n) {
